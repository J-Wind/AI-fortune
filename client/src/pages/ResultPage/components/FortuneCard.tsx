import React from 'react';
import { Card, CardContent } from '@/components/ui/card';
import { FortuneResult } from '@/stores/fortuneStore';

interface FortuneCardProps {
  fortune: FortuneResult;
  cardRef?: React.RefObject<HTMLDivElement>;
}

export function FortuneCard({ fortune, cardRef }: FortuneCardProps) {
  return (
    <div className="relative max-w-md mx-auto" ref={cardRef}>

      {/* 吉凶等级标识 */}
      <div className="text-center mb-4">
        <div className="text-xl font-bold text-primary font-serif relative inline-block">
          {fortune.fortune || '上上签'}
          <div className="absolute -left-8 top-1/2 w-6 h-px bg-primary/40"></div>
          <div className="absolute -right-8 top-1/2 w-6 h-px bg-primary/40"></div>
        </div>
      </div>

      {/* 主签文卡片 */}
      <Card className="relative bg-paper border-2 border-primary/30 shadow-2xl overflow-hidden">
        <div className="absolute inset-0 bg-paper-texture opacity-20 pointer-events-none"></div>
        <div className="absolute inset-0 border-2 border-primary/20 pointer-events-none"></div>
        
        <CardContent className="p-5 relative z-10 flex justify-center items-center h-[380px]">
          <div className="writing-mode-vertical-rl text-start space-y-4">
            <div className="text-lg text-foreground font-serif leading-loose">
              【{fortune.number}】{fortune.mainText}
            </div>
            
            <div className="text-xs text-muted-foreground italic leading-relaxed">
              {fortune.culturalReference}
            </div>
            
            <div className="text-xs text-muted-foreground font-medium">
              {fortune.hexagram}
            </div>
          </div>
          
          <div className="absolute bottom-3 left-3">
            <div className="w-10 h-10 border-2 border-primary/50 rounded-full flex items-center justify-center">
              <div className="text-xs text-primary font-serif">天机</div>
            </div>
          </div>
        </CardContent>
      </Card>


    </div>
  );
}
