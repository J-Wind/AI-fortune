  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <Label htmlFor="thought" className="text-sm font-medium text-left">
          心中所念 <span className="text-destructive">*</span>
        </Label>
        <span className={`text-xs ${isInvalid ? 'text-destructive' : 'text-muted-foreground'} text-left`}>
          {characterCount}/50
        </span>
      </div>
      <Textarea
        ...
        className={`min-h-[80px] resize-none text-sm ${isInvalid ? 'border-destructive' : ''}`}
      />
      {isInvalid && (
